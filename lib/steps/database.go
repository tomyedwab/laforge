...
func (sdb *StepDatabase) CreateStep(step *Step) (int, error) {
	if step == nil {
		return 0, fmt.Errorf("step cannot be nil")
	}
	...
	if step.CommitSHABefore == "" {
		return 0, fmt.Errorf("commit_sha_before is required")
	}

	stepJSON, err := step.ToJSON()
	if err != nil {
		return 0, fmt.Errorf("failed to serialize step: %w", err)
	}

	result, err := sdb.db.Exec(`
		INSERT INTO steps (
			active, parent_step_id, commit_sha_before, commit_sha_after,
			agent_config_name, start_time, end_time, duration_ms,
		...
		stepJSON.AgentConfigName, stepJSON.StartTime, stepJSON.EndTime, stepJSON.DurationMs,
...
func (sdb *StepDatabase) GetStep(stepID int) (*Step, error) {
	...
	err := sdb.db.QueryRow(`
		...
		&stepJSON.CommitSHAAfter, &stepJSON.AgentConfigName, &stepJSON.StartTime,
