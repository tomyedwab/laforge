...
func TestGetStep(t *testing.T) {
	...
	_, err := sdb.CreateStep(originalStep)
	if err != nil {
		t.Fatalf("Failed to create step: %v", err)
	}
	...
	if retrievedStep.AgentConfigName != originalStep.AgentConfigName {
		t.Errorf("Agent config model mismatch: got %s, want %s", retrievedStep.AgentConfigName, originalStep.AgentConfigName)
	}
...
func TestParentStepRelationship(t *testing.T) {
	...
	_, err := sdb.CreateStep(parentStep)
	if err != nil {
