...
class ApiService {
  ...
  async getTask(id: number, include?: {
    children?: boolean;
    logs?: boolean;
    reviews?: boolean;
  }): Promise<{ task: Task }> {
  ...
  // Task logs
  async getTaskLogs(taskId: number, page?: number, limit?: number): Promise<{ logs: TaskLog[]; pagination: any }> {
    const searchParams = new URLSearchParams();
  ...
  async submitReviewFeedback(reviewId: number, feedback: {
